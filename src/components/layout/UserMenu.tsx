...
import { ChevronsUpDown, LogOut, Settings, ShieldCheck, KeyRound, UserRound } from 'lucide-react';
import { useAuth } from '@/contexts/AuthContext';
import { AccountModal } from '../account/AccountModal';
import { TokensModal } from '../account/TokensModal';
import { HealthModal } from '../account/HealthModal';

type ActiveModal = 'account' | 'tokens' | 'health' | null;

/**
 * ChatGPT / Claude style account control at the BOTTOM of the left sidebar.
 * The avatar opens a popover; entries open account modals or navigate to the
 * full Settings page.
 */
export const UserMenu: React.FC<{ onNavigate?: () => void }> = ({ onNavigate }) => {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div className="animate-menu-rise absolute bottom-[calc(100%+0.6rem)] left-0 right-0 z-50 overflow-hidden rounded-2xl border border-emerald-300/20 bg-[#06100B]/95 p-2 shadow-[0_0_60px_rgba(0,255,136,0.18)] backdrop-blur-xl">
          ...
          <button
            ...
            className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition hover:bg-emerald-400/10"
          >
            ...
            <UserRound className="h-4 w-4 shrink-0 text-emerald-300/60" />
          </button>
          ...
          <button className={menuItem} onClick={() => openModal('tokens')}>
            <KeyRound className="h-4 w-4 text-emerald-300" /> Tokens &amp; secrets
          </button>
          <button className={menuItem} onClick={() => openModal('health')}>
      ...
      {/* Account modals (rendered via portal inside each modal) */}
      <AccountModal
        open={modal === 'account'}
        onClose={() => setModal(null)}
        onOpenSettings={() => go('/settings')}
      />
      <TokensModal open={modal === 'tokens'} onClose={() => setModal(null)} />
      <HealthModal
        open={modal === 'health'}
        onClose={() => setModal(null)}
        onOpenFull={() => go('/health')}
      />
    </div>
  );
};
